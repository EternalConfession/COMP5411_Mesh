...
public:
    ...
    /* some supplementary method */
    
    //Travel via loop boundaries
    void LoopBhe(int index);
    //Set flags back to the initial state;
    void SetBheBack();
    //Use BFS on a certain vertex
    void BFSVertex(int index);
    //Set flags back to the initial state;
    void SetVertexBack();
    //Calculate Space of triangle
    double CalculateSpace(Vertex* p, Vertex* p1, Vertex* p2);
    //Calculate Cot of an angle
    double CalculateCot(Vertex* p, Vertex* pm, Vertex* pj);
    //Normalise the meanCurvature;
    void MeanCurvatureNormalise(std::vector<double>& meanList);
    //BuildMatrix
    Eigen::SparseMatrix<double> BuilMatrix(bool uniformWeights);
    Eigen::SparseMatrix<double> BuilMatrix(bool uniformWeights, double Lambda);
    /************************************************************************/
